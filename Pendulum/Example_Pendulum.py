...
class Parameters:
    ...
    t_train: float = 20.0  # Training final time
    t_test: float = 50.0  # Testing final time
    Nt: int = 25  # Number initial conditions of training samples
    Ntest: int = 25  # Number initial conditions of testing samples
    canonical_dim: int = 2 # canonical dimension
    latent_dim: int = 2 # latent canonical dimensional
    hidden_dim: int = 8 # number of neurons in a hidden layer
    smpling_intl = [-3, 3]  # sampling interval
    sample_size: int = 20 # number of samples in a given training time interval
...
if __name__ == "__main__":
    ...
    # Generating data
    dyn_model = data_gen.PendulumExample()
    ...
    plt.savefig(params.path + "pend_phase.png", dpi=300)
    plt.savefig(params.path + "pend_phase.pdf")
