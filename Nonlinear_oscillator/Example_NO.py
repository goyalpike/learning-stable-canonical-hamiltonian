...
class Parameters:
    ...
    t_train = 4  # Training final time
    t_test = 50  # Testing final time
    Nt = 50  # Number initial conditions of training samples
    Ntest = 25  # Number initial conditions of testing samples
    canonical_dim = 2 # canonical dimension
    latent_dim = 2 # latent canonical dimensional
    hidden_dim = 8 # number of neurons in a hidden layer
    smpling_intl = [-2.0, 2.0]  # sampling interval
    sample_size = 20 # number of samples in a given training time interval
    batch_size = 32 # batch size
    max_potential = 1 # max potential to generate training data
    lr = 3e-3  # Learning rate
    encoder = "MLP" # type of neural network
...
if __name__ == "__main__":
    ...
    params.path = "./../Results/NLO/" + params.confi_model + "/"
    ...
    optim = torch.optim.Adam(
        [
            {"params": models["ae"].parameters(), "lr": params.lr, "weight_decay": 1e-5},
            {"params": models["hnn"].parameters(), "lr": params.lr, "weight_decay": 1e-5},
        ]
    )
    ...
    for i in range(params.Ntest):
        ...
        t = np.linspace(t_span[0], t_span[1], 100 * params.Nt)

        # define initial condition
        y0_test = np.random.uniform(params.smpling_intl[0], params.smpling_intl[1], 2)

        while np.abs(dyn_model.potential(y0_test[0], y0_test[1])) > params.max_potential:
            y0_test = np.random.uniform(params.smpling_intl[0], params.smpling_intl[1], 2)
    ...
    plt.ylim(-2, 2)
    plt.xlim(-1.5, 1.5)
    plt.savefig(params.path + "NO_phase.png", dpi=300)
    plt.savefig(params.path + "NO_phase.pdf")
